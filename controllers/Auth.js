...
await sendMail(
  existingUser.email,
  ` Verification OTP pour votre compte FINI PRO `,
  `
  <div style="font-family: 'Arial', sans-serif; background-color: #F0F1EB; padding: 40px 0; text-align: center;">
    <div style="max-width: 600px; margin: 0 auto; background-color: #FFFFFF; border-radius: 20px; box-shadow: 0 25px 50px rgba(0,0,0,0.2); overflow: hidden;">
      
      <!-- Header / Logo -->
      <div style="background: linear-gradient(135deg, #4B6043, #556E51); padding: 40px 20px;">
        <h1 style="margin: 0; font-size: 28px; font-weight: 900; color: #FFFFFF; text-shadow: 0 2px 6px rgba(0,0,0,0.3);">
          FINI PRO
        </h1>
      </div>

      <!-- Content -->
      <div style="padding: 40px 30px; text-align: center; color: #1F1F1F;">
        <p style="font-size: 16px; margin-bottom: 20px;">
          Bonjour, <br>
          Utilisez le code ci-dessous pour vérifier votre compte FINI PRO. 
        </p>

        <div style="font-size: 32px; font-weight: 700; color: #4B6043; margin: 30px 0;">
          ${otp}
        </div>

        <p style="font-size: 14px; color: #556E51; margin-bottom: 0;">
          Ne partagez jamais ce code avec qui que ce soit pour des raisons de sécurité.
        </p>
      </div>

      <!-- Footer -->
      <div style="background-color: #F0F1EB; padding: 20px; font-size: 12px; color: #4B6043;">
        &copy; ${new Date().getFullYear()} FINI PRO. Tous droits réservés.
      </div>
...
exports.forgotPassword = async (req, res) => {
  ...
  try {
    ...
    await sendMail(
      ...
      `
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#F0F1EB;padding:40px 0;font-family:Arial, sans-serif;">
    <tr>
      <td align="center">
        <table width="600" cellpadding="0" cellspacing="0" 
          ...
          <!-- Header -->
          <tr>
            <td style="background:linear-gradient(90deg,#4B6043,#556E51);
          ...
          <!-- Content -->
          <tr>
            <td style="padding:30px 40px;color:#1F1F1F;font-size:16px;line-height:1.6;">
              <p>Bonjour <strong>${isExistingUser.name}</strong>,</p>

              <p>Nous avons reçu une demande de réinitialisation de votre mot de passe pour votre compte Finipro.  
              Si vous êtes à l’origine de cette demande, veuillez cliquer sur le bouton ci-dessous :</p>

              <div style="text-align:center;margin:35px 0;">
                <a href="${process.env.ORIGIN}/reset-password/${isExistingUser._id}/${passwordResetToken}"
                   style="background:linear-gradient(90deg,#4B6043,#556E51);
                          color:#ffffff;padding:14px 28px;text-decoration:none;
                          font-size:17px;border-radius:8px;font-weight:600;
                          display:inline-block;">
                  Réinitialiser mon mot de passe
                </a>
              </div>

              <p>Ce lien est valable pour une durée limitée.  
              Si vous n’avez pas demandé de réinitialisation, vous pouvez ignorer cet e-mail.</p>

              <p style="margin-top:30px;">Merci,<br>
              <strong>L’équipe Finipro</strong></p>
            </td>
          </tr>

          <!-- Footer -->
          <tr>
            <td style="background:#F0F1EB;text-align:center;padding:18px;color:#555;font-size:13px;">
